      ...
      {['left'].map((anchor) => (
        <React.Fragment key={anchor}>
          <Button onClick={toggleDrawer(anchor, true)}>
            {<MenuIcon size="large" edge="start" style={{ color: '#C58944' }} aria-label="menu" sx={{ mr: 2 }} />}
          </Button>
...
function RenderItemsMenu() {
  ...
  return itemsMenu.map((item, index) => {
    return (
      <List key={index}>
        <Link href={linksMenu[index]} underline="none" style={{ color: '#C58944' }}>
          <ListItem key={item} disablePadding>
            <ListItemButton style={{ color: '#C5894' }}>
              <ListItemIcon style={{ color: '#C58944' }}>{iconsMenu[index]}</ListItemIcon>
